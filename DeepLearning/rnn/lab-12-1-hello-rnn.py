...
hidden_size = 5  # output from the LSTM. 5 to directly predict one-hot(output one-hot크기)
batch_size = 1   # one sentence(문자열 몇 개인가)
sequence_length = 6  # |ihello| == 6(문자열 길이 얼마인가)
learning_rate = 0.1

X = tf.placeholder(
    tf.float32, [None, sequence_length, input_dim])  # X one-hot, None은 batch_size로 들어오는대로 하겠다는 뜻
Y = tf.placeholder(tf.int32, [None, sequence_length])  # Y label, None은 batch_size로 들어오는대로 하겠다는 뜻
