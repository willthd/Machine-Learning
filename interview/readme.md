...
기울기 산출 시, 오차역전파법 방식은 각 노드의 미분을 구해 저장하면서 중복 계산을 피할 수 있다. 따라서 수치 미분법보다 빠르고 효율적으로 계산 가능하다.(수치 미분법은 구현하기 더 쉬우며, 오차역전파법으로 구한 기울기(gradient)를 확인하는 데 사용한다.)
...
활성화 함수 : sigmoid, tanh, relu, leaky relu, elu, selu, prelu, maxout

**선형 층을 깊게 쌓아도 여전히 하나의 선형 연산이기 때문에** 층을 여러 개로 구성하는 장점이 없음. 따라서 **가설 공간을 풍부하게 만들기 위해**(확장시키기 위해) **비선형성을 갖는 활성화 함수**를 추가(Keras, p.110)
