...
좋은 머신러닝을 설계하는 것은 최적화와 일반화 사이의 줄다리기이며, 진짜 문제는 일반화이다. 드롭아웃 등을 사용해 **과대적합을 줄이는 기본 단계를 거친 후 과대적합이 일어날 때 까지 네트워크의 용량을 늘리는 것**이 좋다
...
3. 가중치 규제
   ...
   * **L1 규제** : Loss function에 가중치의 절댓값에 비례하는 비용 추가. 일부 가중치 값을 0으로 만들 수 있다 
   * **L2 규제** : Loss function에 가중치의 제곱에 비례하는 비용 추가. **가중치 감쇠**라고도 한다
   * ElsticNet : L1, L2 규제를 함께 사용하는 것을 의미한다. 3가지 규제는 모두 테스트가 아닌 훈련시에만 추가된다
