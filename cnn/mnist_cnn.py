...
X_img = tf.reshape(X, [-1, 28, 28, 1])   # img 28x28x1 (black/white), -1 : N개
Y = tf.placeholder(tf.float32, [None, 10])

# L1 ImgIn shape=(?, 28, 28, 1)
W1 = tf.Variable(tf.random_normal([3, 3, 1, 32], stddev=0.01))  # 3x3x1 filter 32개
#    Conv     -> (?, 28, 28, 32)
#    Pool     -> (?, 14, 14, 32)
# stride 크기는 1x1(가운데 두 수), padding = same은 convLayer와 사이즈 똑같이 28x28로 만들겠다는 의미
# [batch_size, image_rows, image_cols, number_of_colors]
L1 = tf.nn.conv2d(X_img, W1, strides=[1, 1, 1, 1], padding='SAME')
L1 = tf.nn.relu(L1)
L1 = tf.nn.max_pool(L1, ksize=[1, 2, 2, 1],
                    strides=[1, 2, 2, 1], padding='SAME')   # ksize는 filter_size와 동일
'''
...
# 위의 32 그대로 내려와야 한다. 크기는 3x3, filter 개수는 64
...
# Final FC 7x7x64 inputs -> 10 outputs(0~9까지)
W3 = tf.get_variable("W3", shape=[7 * 7 * 64, 10],
                     initializer=tf.contrib.layers.xavier_initializer())
# 출력의 10이 그대로 bias로 내려온다
